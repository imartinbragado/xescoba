...
void coge(char carta, jugador &quien)
{
  ...
  for (int i = 0; i < 40; i++)
    if (centro[i] == carta)
      centro[i] = -1; //Si es la carta... desaparece del mont�n.
...
void pon_centro(jugador *quien)
{
  ...
  if (!viejo)
  { /* Si es la primera vez que entra inicializa la estructura */
    for (i = 0; i < 40; i++)
    {
      naipes_boton[i] = NULL;
      dibujo[i] = NULL;
  ...
  /* C�digo a�adido el 26 del 12 del 98 para sugerir la jugada */
  if (global.sugerencias && quien->mi_naturaleza() == HUMANO)
  {
    const char *lleva = quien->mi_mano();
    char guay[40];
    const char *p;
    ...
    for (i = 0; i < 40; i++)
    {
      ...
      {
        //Si tengo esta carta.
        p = piensa(lleva[i], centro); //�Cu�l es la mejor jugada
    ...
    for (i = 0; i < 40; i++)
      for (j = 0; j < 40; j++)
      {
        if (centro[i] != -1 && centro[i] == guay[j]) //La carta mano[i] est� escogida
        {
...
void pon_mano(jugador *quien)
{
  ...
  for (i = 0; i < 40; i++)
    ...
    { //Si existe una carta
      ...
      if (!global.sugerencias)
        ...
        } //Con esto elijo un bot�n por defecto y actualizo la selecci�n.
      gtk_signal_connect(GTK_OBJECT(naipes_boton[i]),
                         "clicked",
  ...
  if (global.sugerencias)
  {
    ...
    for (int i = 0; i < 40; i++)
    {
      ...
      {
        //Si tengo esta carta.
        p = piensa(lleva[i], centro); //�Cu�l es la mejor jugada con esta?
        if (puntuacion > punt_guay)
        {
          memcpy(guay, p, 40);
          j = i;
...
int gentetonum(jugador &quien)
{
  ...
  if (!banderita)
  {
    CONF conf(global.FICH_CONF); /* Abro la configuraci�n */
    char cad[80];
    for (int i = 0; i < 4; i++)
    {
