...
/* Cada vez que se pulsa este bot�n debe pasar un turno... */
{
   ...
   if ((intptr_t)data == TURNO_EMPEZAR) { /* Esto supone que hay que repartir el juego... */
      cartas = new baraja;
	...
	suma_centro+=CARTA(centro[i]);
	 ...
	 for (i=0;i<4;i++)
	   if (CARTA(centro[i]) <8 )
	     menor8++;
	 if (menor8 == 2) { /* S�lo si ha hecho dos escobas */
	    for (i=0;i<4;i++)
	      coge(centro[i],gente[num_jug-1]);
	    gente[num_jug-1].hizo_escoba();
	    sprintf(temporal, _(" has won two sweeping") ,
   ...
   if( cartas->quedan() || ban ) { //Mientras queden cartas
      
      /* Reparto tres cartas a cada jugador */
      int j; /* Contador para cada jugador */
      if (!ban ) { // Si no quedan cartas a nadie 
	 suena(SONIDO_REPARTIR);
...
{
   char *p = getenv("HOME");
   static char * _FICH_CONF = ".xescoba";
   if (p == NULL)
